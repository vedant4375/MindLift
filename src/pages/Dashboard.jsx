...
const CoachWidget = () => {
  ...
  const [messages, setMessages] = useState(
    savedUser?.chatHistory?.length
      ? savedUser.chatHistory
      : [
          {
            role: "ai",
            text: "Hello! I'm VitaCoach AI. How can I help you today?",
          },
        ]
  );
  const getAIResponse = (message) => {
    ...
    if (text.includes("stress")) {
      return "Your stress levels seem high. Try a 5-minute breathing exercise from the Relax section.";
    }
  
    if (text.includes("sleep")) {
      return "Aim for 7-8 hours of sleep. Avoid screens 30 minutes before bedtime.";
    }
  
    if (text.includes("hydration") || text.includes("water")) {
      return "Hydration is important. Try drinking at least 2-3 liters of water daily.";
    }
  
    if (text.includes("exercise") || text.includes("workout")) {
      return "A 30-minute walk or light workout can improve both physical and mental health.";
    }
    ...
    if (text.includes("anxiety")) {
      return "Take slow deep breaths. Inhale for 4 seconds, hold for 4, exhale for 6.";
    }
  
    if (text.includes("hello") || text.includes("hi")) {
      return "Hello! How can I help you improve your wellness today?";
    }
  
    return "I understand. Could you tell me a little more about your situation?";
  };
...
const handleSend = () => {
  ...
  setTimeout(() => {
    ...
    const savedUser = getUserData();
    savedUser.chatHistory = finalMessages;
...
export default function Dashboard() {
  ...
  const [hydration, setHydration] = useState(
    userData?.hydrationHistory?.length
      ? userData.hydrationHistory[userData.hydrationHistory.length - 1].amount
      : metrics.hydration.current
  );

  const [moodValue, setMoodValue] = useState(
    userData?.moodHistory?.length
      ? userData.moodHistory[userData.moodHistory.length - 1].value
      : 80
  );
...
const addWater = () => {
  setHydration((prev) => {
    const newHydration = Math.min(
      prev + 250,
      data.hydration.target
    );

    const user = getUserData();

    user.hydrationHistory.push({
      amount: newHydration,
